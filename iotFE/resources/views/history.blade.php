                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Lịch sử hoạt động</strong>
                            </div>
                            <form action="{{ route('history', ['pageNumber' => 0]) }}" method="GET">
                                @csrf
                                <div class="input-group col-lg-6" style="margin: 0.5em">
                            ...
                            <div class="table-stats order-table ov-h">
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            <th class="serial">STT <a href="{{ route('history', [$data->pageable->pageNumber, 'id', 'asc']) }}">&#9650;</a> <a href="{{ route('history', [$data->pageable->pageNumber, 'id', 'desc']) }}">&#9660;</a></th>
                                            <th>Thiết bị <a href="{{ route('history', [$data->pageable->pageNumber, 'led', 'asc']) }}">&#9650;</a> <a href="{{ route('history', [$data->pageable->pageNumber, 'led', 'desc']) }}">&#9660;</a> </th>
                                            <th >Thời gian <a href="{{route('history', [$data->pageable->pageNumber, 'created', 'asc']) }}">&#9650;</a> <a href="{{ route('history', [$data->pageable->pageNumber, 'created', 'desc']) }}">&#9660;</a> </th>
                                            <th>Hành động <a href="{{route('history', [$data->pageable->pageNumber, 'action', 'asc']) }}">&#9650;</a> <a href="{{ route('history', [$data->pageable->pageNumber, 'action', 'desc']) }}">&#9660;</a></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($data->content as $item)
                                            <tr>
                                                ...
                                                @endif
                                                <td>{{ \Carbon\Carbon::parse($item->created)->format('Y-m-d h:m:s')}}</td>
                                                @if ($item->action == 1)
                                                    <td>
                                                        <span class="badge badge-complete">Bật</span>
                                                    </td>
                                                @endif
                                                @if ($item->action == 0)
                                                    <td>
                                                        <span class="badge badge-pending">Tắt</span>
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
                                    ...
                                    <ul class="pagination">
                                        @if ($data->totalPages > 1)
                                            ...
                                            <li class="page-item {{ $data->pageable->pageNumber == 1 ? 'disabled' : '' }}">
                                                <a class="page-link" href="{{ route('history', ['pageNumber' => $data->pageable->pageNumber - 1]) }}" aria-label="Previous">
                                            ...
                                            @if ($data->totalPages <= 5)
                                                @for ($index = 0; $index <= $data->totalPages; $index++)
                                                    <li class="page-item {{ $index == $data->pageable->pageNumber ? 'active' : '' }}">
                                                        <a class="page-link" href="{{ route('history', ['pageNumber' => $index]) }}">
                                                            <span>{{ $index + 1 }}</span>
                                                        </a>
                                                    </li>
                                                @endfor
                                            @else
                                                ...
                                                @if ($data->pageable->pageNumber < $data->totalPages - 2)
                                                    ...
                                                    <li class="page-item">
                                                        <a class="page-link" href="{{ route('history', ['pageNumber' => $data->totalPages]) }}">
